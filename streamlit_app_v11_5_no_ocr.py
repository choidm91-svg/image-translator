...
# UI
# ------------------------------------------------------------
st.title("🌐 AI 상세페이지 번역기 v11.5")
st.caption(
    "원클릭 실무 모드: 국가 선택 → 번역 → 검수 필요한 문장만 확인 → 전체 다운로드"
)
st.success("✅ v11.5 NO-OCR MODE · Tesseract를 사용하지 않습니다")
...
def translate_detail_image(image_url, target_language, target_market, dictionary, safety_mode, short_mode):
    safety_instruction = (
...
15. review_required=true 로 표시할 조건:
   ...
   - 이미지 판독이 불확실함
16. 위 조건이 없으면 review_required=false.
17. 설명/해설 없이 JSON만 반환하세요.
"""
        ...
        cleaned.append(
            {
                ...
                "type": seg_type,
                "review_required": bool(reasons) or ai_review,
...
# Run translation
# ------------------------------------------------------------
if "v114_results" not in st.session_state:
    st.session_state.v114_results = []
...
if uploaded_files:
    if st.button("🚀 원클릭 전체 번역", type="primary", use_container_width=True):
        ...
        st.session_state.v114_results = results
        st.success("✅ 번역 완료")

# ------------------------------------------------------------
# Results
# ------------------------------------------------------------
if st.session_state.v114_results:
    total_segments = sum(len(item.get("segments", [])) for item in st.session_state.v114_results)
    total_review = sum(
        1
        for item in st.session_state.v114_results
    ...
    if total_long > 0:
        if st.button("✂️ 긴 문장만 자동 축약", use_container_width=True):
            with st.spinner("길이가 긴 번역만 다시 짧게 줄이고 있습니다..."):
                ...
                st.session_state.v114_results = updated_results
            st.rerun()
