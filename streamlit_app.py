...
# UI
# ------------------------------------------------------------
st.title("🌐 AI 상세페이지 번역기 v11.8")
st.caption(
    "원클릭 실무 모드: 국가 선택 → 번역 → 검수 필요한 문장만 확인 → 전체 다운로드"
)
st.success("✅ v11.8 RESILIENT AUTO-SPLIT · NO-OCR · 빈 결과 자동 재시도")
...
def split_tall_image(image: Image.Image, max_height: int = 3200, overlap: int = 260):
    """
    세로로 매우 긴 상세페이지를 자동 분할합니다.
    각 조각은 약 260px 겹치게 잘라 경계에 걸린 문장 누락을 줄입니다.
    """
...
def parse_json_response(text):
    """JSON 앞뒤에 설명/코드펜스가 섞여도 최대한 구조를 복구합니다."""
    ...
    # 마지막 안전망: segments 배열만 잘라서 복구
    match = re.search(r'"segments"\s*:\s*(\[.*\])', cleaned, re.S)
    if match:
        try:
            return {"segments": json.loads(match.group(1))}
        except Exception:
            pass
...
def translate_chunk_resilient(
    ...
    """
    한 조각에서 segments가 0개면 자동으로 더 작게 나눠 재시도합니다.
    사용자가 직접 이미지를 자를 필요가 없습니다.
    """
    ...
    # 빈 결과일 때만 최대 2단계 재분할
    if depth >= 2 or chunk_image.height <= 1200:
        return [], [raw_response], {
            "status": "empty",
            "height": chunk_image.height,
            "retries": depth,
    ...
    overlap = min(180, max(80, chunk_image.height // 12))
    middle = chunk_image.height // 2
    first_end = min(chunk_image.height, middle + overlap)
    second_start = max(0, middle - overlap)

    sub_images = [
        chunk_image.crop((0, 0, chunk_image.width, first_end)).convert("RGB"),
        chunk_image.crop((0, second_start, chunk_image.width, chunk_image.height)).convert("RGB"),
    ]

    all_segments = []
    all_raw = [raw_response]
    child_reports = []

    for sub_image in sub_images:
        child_segments, child_raw, child_report = translate_chunk_resilient(
            sub_image,
            target_language,
            target_market,
            dictionary,
    ...
    return all_segments, all_raw, {
        "status": "recovered" if all_segments else "empty",
        "height": chunk_image.height,
        "retries": depth + 1,
...
def translate_detail_image_auto_split(
    ...
    """
    긴 상세페이지를 자동 분할하고, 빈 조각은 더 작게 자동 재분할하여 다시 읽습니다.
    """
    chunks = split_tall_image(image, max_height=3200, overlap=260)
    segment_groups = []
    raw_responses = []
    chunk_reports = []
    ...
    for chunk in chunks:
        ...
        segments, raw_list, report = translate_chunk_resilient(
            chunk_image,
            target_language,
            target_market,
            dictionary,
        ...
        report["chunk_index"] = chunk["index"]
        report["y1"] = chunk["y1"]
        report["y2"] = chunk["y2"]

        segment_groups.append(segments)
        raw_responses.extend(raw_list)
    ...
    return (
        merged,
        "\n\n--- CHUNK RESPONSE ---\n\n".join(str(x or "") for x in raw_responses),
        len(chunks),
        chunk_reports,
    )
...
if st.session_state.get("app_version") != APP_VERSION:
    ...
    st.session_state.v118_results = []

if "v118_results" not in st.session_state:
    st.session_state.v118_results = []
    ...
    if st.button("🚀 원클릭 전체 번역", type="primary", use_container_width=True):
        ...
        for idx, uploaded_file in enumerate(uploaded_files, start=1):
            with st.spinner(f"{idx}/{len(uploaded_files)} · {uploaded_file.name} 번역 중..."):
                ...
                try:
                    segments, raw_response, chunk_count, chunk_reports = translate_detail_image_auto_split(
                        image,
                        target_language,
                        selected_market,
                        st.session_state.translation_dictionary,
        ...
        st.session_state.v118_results = results
        zero_files = [item for item in results if not item.get("segments")]
        if zero_files:
            st.warning(
                f"⚠ 번역은 끝났지만 {len(zero_files)}개 이미지에서 문장을 추출하지 못했습니다. "
                "아래 진단 정보를 확인해 주세요."
            )
        else:
            st.success("✅ 번역 완료")
...
# Results
# ------------------------------------------------------------
if st.session_state.v118_results:
    total_segments = sum(len(item.get("segments", [])) for item in st.session_state.v118_results)
    total_review = sum(
        1
        for item in st.session_state.v118_results
    ...
    if total_long > 0:
        if st.button("✂️ 긴 문장만 자동 축약", use_container_width=True):
            with st.spinner("길이가 긴 번역만 다시 짧게 줄이고 있습니다..."):
                ...
                st.session_state.v118_results = updated_results
            st.rerun()
    ...
    with tab_all:
        for item in st.session_state.v118_results:
            ...
            with col_image:
                ...
                st.caption(
                    f"자동 분할: {item.get('chunk_count', 0)}개 · "
                    "빈 조각은 자동 재분할 후 재시도"
                )
            ...
            with col_result:
                ...
                if not segments:
                    st.error("문장 추출 0건 · 완료로 처리하지 않았습니다.")
                    st.caption(
                        f"자동 분할 {item.get('chunk_count', 0)}개 · "
                        f"각 조각은 빈 결과일 경우 최대 2회 더 작게 재분할했습니다."
                    )
                    with st.expander("🔧 조각별 진단", expanded=True):
                        st.json(item.get("chunk_reports", []))
                    with st.expander("AI 원본 응답 보기", expanded=False):
                        st.text_area(
                            "AI 원본 응답",
